...
class BiGG(object):
    ...
    def __init__(self,MODEL_JSON_FILE):
        self.org_model, self.ecoli_S = my_settings.get_org_json(MODEL_JSON_FILE)
    ...
    def _get_metabolite_df():
        bigg2chebi = []
        with open(my_settings.BIGG_METABOLITE_FNAME, 'r') as fp:
    ...
    def _get_reaction_df():
        ...
        with open(my_settings.BIGG_REACTION_FNAME, 'r') as fp:
            csv_reader = csv.reader(fp, delimiter='\t')
            next(csv_reader)
            for row in csv_reader:
                bigg_id = row[0].lower()
    ...
    def get_native_EC_numbers(self):
        #model, _ = my_settings.get_org_json()
        # make a list of all the EC numbers which are mapped to a BiGG reaction
        # in each organism model. Original code is picky for only ones that have genes mapped to them
        rids_with_genes = set()
        for d in self.org_model['reactions']:
            rid = d['id']
    ...
    def get_mets_in_cytosol(self):
        ...
        for d in self.org_model['metabolites']:
            met = d['id']
    ...
    def get_reaction_subsystems(self):
        bigg2subsystem = {}
        for r in self.org_model['reactions']:
            rid = r['id'].lower()
            if 'subsystem' in r:
