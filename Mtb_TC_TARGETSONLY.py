...
def fingerprint_and_tanimoto(mol_list,targetsonly=False):
    ...
    elif targetsonly == True:
        #Specify targets - last 6 entries in spreadsheet with InChiKeys and InChis
        #print(mol_list[-6:])
        targs = fps_rdk[-6:]
        for i in range(len(targs)):
            for j in range(0,len(fps_rdk)):
                if mol_list[i] == mol_list[j]:
                    continue
                else:
                    temp_dict = dict([("InChiKeys",[Chem.InchiToInchiKey(Chem.MolToInchi(mol_list[-6+i])),Chem.InchiToInchiKey(Chem.MolToInchi(mol_list[j]))]),("Tanimoto_coeff",DataStructs.FingerprintSimilarity(targs[i],fps_rdk[j]))])
                    tanimotos.append(temp_dict)
    
    else:
        #throw an error
        raise ValueError("targetsonly must be True or False")
    ...
    out = open(r'C:\Github\pythonScripts\Mtb_inhibition\Mtb_inhibition\tanimoto_output_targets.txt','w')
    for t in tanimotos:
...
def main():
    ...
    if source == 'InChi':
        ...
        mol_list = inchiToMol('C:\Github\pythonScripts\Mtb_inhibition\Mtb_inhibition\data\Mtb_iNJ661_inchis_and_inchikeys.xlsx',0)
    ...
    fingerprint_and_tanimoto(mol_list,targetsonly=True)	
